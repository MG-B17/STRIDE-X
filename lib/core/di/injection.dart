...
Future<void> initDependencies() async {
  //Cubit
  init.registerLazySingleton<StepController>(() => StepController(getStepsUsecase: init()));
  init.registerLazySingleton<CalibrationCubit>(() =>CalibrationCubit(calibrateUseCase: init(), calibrationRepository: init(), calibrationStreamUsecase: init()));


  //UseCase
  init.registerLazySingleton(() => GetStepsUsecase(repository: init()));
  init.registerLazySingleton<CalibrateStepsUseCase>(() => CalibrateStepsUseCaseImpl());
  init.registerLazySingleton<CalibrationStreamUsecase>(() => CalibrationStreamUsecase(repository: init()));

  //Repository
  init.registerLazySingleton<StepRepository>(() => StepRepositoryImpl(dateHelper: init(), stepCounterLocalData: init(), baselineLocalData: init(), userStepGoalLocal: init()));
  init.registerLazySingleton<CalibrationRepository>(() => CalibrationRepositoryImpl(localData: init()));
  init.registerLazySingleton<CalibrationStreamRepository>(() => CalibrationStreamRepositoryImpl());

  // DataSource
  init.registerLazySingleton<BaslineLocalData>(()=> BaslineLocalDataImpl());
  init.registerLazySingleton<UserStepGoalLocal>(()=> UserstePGoalLocalImple());
  init.registerLazySingleton<StepCounterLocalData>(()=> StepCounterLocalDataImpl(databaseService: init(), dateHelper: init()));
  init.registerLazySingleton<DateHelper>(()=> DateHelperImpl());
  init.registerLazySingleton<CalibrationLocalData>(()=> CalibrationLocalDataImpl());
